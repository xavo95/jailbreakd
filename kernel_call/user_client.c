...
stage2_copyout_user_client_vtable() {
	...
	original_vtable = rk64(user_client);
	...
	kread(original_vtable_xpac, vtable_contents, max_vtable_size);
...
stage2_patch_user_client(uint64_t *vtable, size_t count) {
	...
	kwrite(kernel_buffer, vtable, count * sizeof(*vtable));
	...
	uint64_t reserved = rk64(reserved_field);
	...
	wk64(user_client, vtable_pointer);
}
...
stage2_unpatch_user_client() {
	...
	wk64(user_client, original_vtable);
}
...
stage1_kernel_call_deinit() {
	if (trap != 0) {
		...
		kwrite(trap, trap_data, SIZE(IOExternalTrap));
		trap = 0;
	}
	if (kernel_buffer != 0) {
